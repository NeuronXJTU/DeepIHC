...
class COCODetection(data.Dataset):
    ...
    def pull_item(self, index):
        ...
        return np.transpose(image, [2, 0, 1]), boxes, masks,boundarys, num_crowds
...
def dataset_collate(batch):
    ...
    for sample in batch:
        ...
        masks.append(torch.from_numpy(sample[2].copy()))
        boundarys.append(torch.from_numpy(sample[3].copy()))
        num_crowds.append(sample[4])
    #targets=[ann.cuda() for ann in targets]
    #masks=[mask.cuda() for mask in masks]
    #boundarys=[boundary.cuda() for boundary in boundarys]
