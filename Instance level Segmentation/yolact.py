...
class YOLACT(object):
    ...
    def detect_image(self, image):
        ...
        with torch.no_grad():
           ...
           # print("==============outputs",outputs)
            ...
            if results[0] is None:
                return image
            box_thre, class_thre, class_ids, masks_arg, masks_sigmoid,boundarys_arg = [x.cpu().numpy() for x in results]
        ...
        image_fused1     = cv2.addWeighted(color_masks, 0.4, image_origin, 0.6, gamma=0)
        for i in range(np.shape(class_ids)[0]):
            ...
            cv2.rectangle(image_fused1, (left, top), (right, bottom), color, thickness)
            ...
            cv2.rectangle(image_fused1, (left, top), (left + text_w, top + text_h + 5), color, -1)
            cv2.putText(image_fused1, text_str, (left, top + 15), font, scale, (255, 255, 255), 1, cv2.LINE_AA)

        image1 = Image.fromarray(np.uint8(image_fused1))
        return image1
    ...
    def eval_image(self, image):
        ...
        mask_8[mask_8>0]=1
        boundarys_arg[boundarys_arg>0]=1

        return mask_8, boundarys_arg
